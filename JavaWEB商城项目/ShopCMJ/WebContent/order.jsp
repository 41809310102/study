			...
			<div class="shop_meber_bd_good_lists clearfix">
				...
				<table>
					<thead class="tab_title">
						...
						<th style="width:100px;"><span>小计</span></th>
					...
					<tbody>
						...
						<tr><td colspan="5">
							<table class="good">
								<thead >
									<tr><th colspan="6">
										<span><strong>订单号码：</strong>${o.oid }</span>
									</th></tr>
								</thead>
								<tbody>
								<c:forEach items="${o.list}" var="item">
									...
									<tr>
										<td class="dingdan_pic"><img src="${pageContext.request.contextPath}/${item.product.pimage}" /></td>
										<td class="dingdan_title"><span><a href="">${item.product.pname}</a></span></td>
										<td class="dingdan_danjia">￥<strong>${item.product.price}</strong></td>
										<td class="dingdan_shuliang">${item.count}</td>
										<td class="dingdan_zongjia">￥<strong>${item.subtotal}</strong><br />(免运费)</td>
										<td class="digndan_caozuo"><c:if test="${o.state==1}">
										<span>已付款</span><a href="javaScript:void(0)">评价</a>
										</c:if>
										<c:if test="${o.state==0}">
